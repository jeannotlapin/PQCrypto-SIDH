...
#define SIKEp503_SECRETKEYBYTES     434    // MSG_BYTES + SECRETKEY_B_BYTES + SIKEp503_PUBLICKEYBYTES bytes
#define SIKEp503_PUBLICKEYBYTES     378
#define SIKEp503_SHAREDBYTES         16
#define SIKEp503_CIPHERTEXTBYTES    402    // SIKEp503_PUBLICKEYBYTES + MSG_BYTES bytes  

// Algorithm name
#define SIKEp503_ALGNAME "SIKEp503"  

// SIKE's key generation
// It produces a private key sk and computes the public key pk.
// Outputs: secret key sk (SIKEp503_SECRETKEYBYTES = 434 bytes)
//          public key pk (SIKEp503_PUBLICKEYBYTES = 378 bytes) 
SIDH_PUBLIC int crypto_kem_keypair_SIKEp503(unsigned char *pk, unsigned char *sk);

// SIKE's encapsulation
// Input:   public key pk         (SIKEp503_PUBLICKEYBYTES = 378 bytes)
// Outputs: shared secret ss      (SIKEp503_SHAREDBYTES = 16 bytes)
//          ciphertext message ct (SIKEp503_CIPHERTEXTBYTES = 402 bytes)
SIDH_PUBLIC int crypto_kem_enc_SIKEp503(unsigned char *ct, unsigned char *ss, const unsigned char *pk);

// SIKE's decapsulation
// Input:   secret key sk         (SIKEp503_SECRETKEYBYTES = 434 bytes)
//          ciphertext message ct (SIKEp503_CIPHERTEXTBYTES = 402 bytes) 
// Outputs: shared secret ss      (SIKEp503_SHAREDBYTES = 16 bytes)
SIDH_PUBLIC int crypto_kem_dec_SIKEp503(unsigned char *ss, const unsigned char *ct, const unsigned char *sk);
...
#define SIDHp503_SECRETKEYBYTES      32
#define SIDHp503_PUBLICKEYBYTES     378
#define SIDHp503_SHAREDBYTES        126
...
SIDH_PUBLIC void random_mod_order_A_SIDHp503(unsigned char* random_digits);
...
// Outputs random value in [0, 2^250 - 1] to be used as Alice's private key
SIDH_PUBLIC void ext_random_mod_order_A_SIDHp503(unsigned char* random_digits, int (*rngFunction)(void *, unsigned char *, long unsigned int), void *rngContext);

// Generation of Bob's secret key 
// Outputs random value in [0, 2^Floor(Log(2,3^159)) - 1] to be used as Bob's private key
SIDH_PUBLIC void random_mod_order_B_SIDHp503(unsigned char* random_digits);
// Generation of Bob's secret key using external RNG
// the external RNG prototype must be rngFunction(void *context, unsigned char *output_buffer, long unsigned int output_size) - context can be null if not needed
// Outputs random value in [0, 2^Floor(Log(2,3^159)) - 1] to be used as Bob's private key
SIDH_PUBLIC void ext_random_mod_order_B_SIDHp503(unsigned char* random_digits, int (*rngFunction)(void *, unsigned char *, long unsigned int), void *rngContext);
...
SIDH_PUBLIC int EphemeralKeyGeneration_A_SIDHp503(const unsigned char* PrivateKeyA, unsigned char* PublicKeyA);
...
SIDH_PUBLIC int EphemeralKeyGeneration_B_SIDHp503(const unsigned char* PrivateKeyB, unsigned char* PublicKeyB);
...
SIDH_PUBLIC int EphemeralSecretAgreement_A_SIDHp503(const unsigned char* PrivateKeyA, const unsigned char* PublicKeyB, unsigned char* SharedSecretA);
...
SIDH_PUBLIC int EphemeralSecretAgreement_B_SIDHp503(const unsigned char* PrivateKeyB, const unsigned char* PublicKeyA, unsigned char* SharedSecretB);
